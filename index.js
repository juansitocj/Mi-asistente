...
async function manejarComandoAdmin(sock, from, texto) {
    ...
    if (cmd.startsWith('!permitir ')) {
        ...
        lista.push(numero);
        guardarJSON('lista_blanca.json', lista);
        return `✅ *${numero}* agregado a lista blanca antilink.`;
    }

    if (cmd.startsWith('!quitar ')) {
        const numero = texto.split(' ')[1].replace(/[^0-9]/g, '');
        let lista = cargarJSON('lista_blanca.json').filter(n => n !== numero);
        guardarJSON('lista_blanca.json', lista);
    ...
    if (cmd === '!encender') {
        ...
        if (perdidos.length === 0) {
            resumen += 'No recibiste mensajes mientras estaba apagado.';
        } else {
        ...
        if (estado.paso === 2) {
            if (cmd === '!enviar') {
                ...
                for (const c of destinatarios) {
                    try {
                        await sock.sendMessage(c.numero, { text: estado.mensaje });
                        enviados++;
    ...
    if (cmd === '!contactos') {
        ...
        contactos.forEach((c, i) => {
            res += `${excluidos.includes(c.numero) ? '🚫' : '✅'} ${i + 1}. ${c.numero}\n`;
        });
        return res;
    }

    if (cmd === '!stats') {
        const citas2 = cargarJSON('citas.json');
        const contactos = cargarJSON('contactos.json');
        return `📊 *ESTADÍSTICAS:*\n\n📅 Citas totales: *${citas2.length}*\n✅ Confirmadas: *${citas2.filter(c => c.estado === 'confirmada').length}*\n⏳ Pendientes: *${citas2.filter(c => c.estado === 'pendiente').length}*\n❌ Canceladas: *${citas2.filter(c => c.estado === 'cancelada').length}*\n👥 Contactos: *${contactos.length}*`;
    }
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
        ...
        for (const msg of messages) {
            ...
            if (!botActivo && !esAdmin) {
                ...
                await enviar('⏸️ En este momento estoy ocupado, escribe más tarde. ¡Gracias por tu paciencia! 😊');
                continue;
            }

            // Registrar contacto
            registrarContacto(from.replace('@s.whatsapp.net', ''));

            // AGENDAMIENTO EN CURSO
            if (esperandoCita[from]) {
                await enviar(await procesarAgendamiento(sock, from, texto));
                continue;
            }

            // INICIAR AGENDAMIENTO
            if (texto.toLowerCase().includes('cita') || texto.toLowerCase().includes('agendar')) {
                await enviar(await procesarAgendamiento(sock, from, texto));
                continue;
            }
            ...
            await enviar(await responderConIA(from, texto));
        }
    });
}
